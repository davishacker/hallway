...
exports.account = function(id, app, entries, cbDone)
{
  ...
  // fetch the reqired auth object
  profileManager.authGet(pid, app, function(err, auth){
    if(err) return cbDone(err);
    if(!auth) { // we need to spoof a blank one here for the push pump, this should get refactored away someday
      auth = {apps:{}};
      auth.apps[app] = {accounts:{}};
      auth.apps[app].accounts[id] = true;
      auth.pid = pid;
    }
    pumper(entries, [app], auth, cbDone);
  });
};
